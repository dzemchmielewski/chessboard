...
class MFRC522(Common):
    ...
    def __init__(self, sck, mosi, miso, rst, cs_handler, baudrate=1000000, spi_id=0):
        super().__init__("MFRC522", True, True)
    ...
    def cs_start(self):
        self.cs_handler.start()

    def cs_stop(self):
        self.cs_handler.stop()
    ...
    def read_uid(self):
        ...
        if stat == self.OK:
            (stat, uid) = self.SelectTagSN()
            if stat == self.OK:
                result = uid
        return "-".join("{:02x}".format(x) for x in result).upper() if result else ""

    def _wreg(self, reg, val):
        # metering = Metering()
        self.cs_start()
        # self.log("(WREG 1) {}".format(metering.step()))
        self.spi.write(b'%c' % int(0xff & ((reg << 1) & 0x7e)))
        # self.log("(WREG 2) {}".format(metering.step()))
        self.spi.write(b'%c' % int(0xff & val))
        # self.log("(WREG 3) {}".format(metering.step()))
        self.cs_stop()
        # self.log("(WREG 4) {}".format(metering.step()))
    ...
    def _tocard(self, cmd, send):
        ...
        # self.log("TOCARD1 {}".format(metering.step()))
        #i = 2000
        i = 25
        while True:
            n = self._rreg(0x04)
        ...
        #self.log("TOCARD2 {}".format(metering.step()))
        ...
        #self.log("TOCARD END i = {}, {}".format(i, metering.step()))
